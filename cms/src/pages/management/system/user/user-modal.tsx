...
export function UserModal({
  ...
  const [checkedDepartmentKeys, setCheckedDepartmentKeys] = useState<any>([]);
  ...
  const getDepartments = async () => {
    ...
    const res: any = await department.searchItem(param);
  ...
  useEffect(() => {
    ...
    setCheckedDepartmentKeys(
      formValue?.Departments.map((item: any) => {
        return item.Id;
      })
    );
  }, [formValue, form]);
  ...
  const onCheckDepartments = (checkedKeys: any) => {
    setCheckedDepartmentKeys(checkedKeys);
  };
  const onFinish = async (values: any) => {
    values.RoleIds = checkedKeys;
    values.UserId = formValue.UserId;
    values.DepartmentIds = checkedDepartmentKeys;
    let res = {
      Status: false,
    };
    if (title != "Chỉnh sửa") {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Form.Item label="Duyệt" name="Workflow" required>
          <Radio.Group optionType="button" buttonStyle="solid">
            <Radio value={1}>Chờ duyệt </Radio>
            <Radio value={-1}> Hủy</Radio>
            <Radio value={4}> Duyệt</Radio>
          </Radio.Group>
        </Form.Item>
        ...
        <Form.Item label="Phòng ban" name="Departments">
          <Tree
            checkable
            checkedKeys={checkedDepartmentKeys}
            defaultCheckedKeys={checkedDepartmentKeys}
